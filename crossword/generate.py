...
class CrosswordCreator():
        ...
        for var in self.crossword.variables:
            for word in set(self.domains[var]):
                if len(word) != var.length:
                    # if the word is smaller or bigger than variable length, 
    ...
    def revise(self, x, y):
        ...
        for x_w in self.domains[x].copy():
            remove = True
            for y_w in self.domains[y]:
                ...
                if remove and x_w in self.domains[x]:
                    self.domains[x].remove(x_w)
                    revision = True
        return revision
    ...
    def consistent(self, assignment):
        ...
        for var in assignment.keys():
            if assignment[var] in words_present or len(assignment[var]) != var.length:
                return False
            else:
                words_present.add(assignment[var])
    ...
    def order_domain_values(self, var, assignment):
        ...
        return list(val[0] for val in count)
