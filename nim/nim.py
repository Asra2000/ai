...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if not epsilon:
            ...
            if prob <= self.epsilon:
                actions = [action for action in Nim.available_actions(state) if (tuple(state), action) not in self.q]
    ...
    def best_move(self, state):
        ...
        best_action = None
        for action in Nim.available_actions(state):
                q_val = self.get_q_value(state, action) if (tuple(state), action) in self.q else 0
